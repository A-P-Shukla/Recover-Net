...
from recover_net.core.security import (
    MaskingError,
    clear_logger_cache,
    get_blind_logger,
    mask_email,
...
def test_require_secret_fails_when_missing(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_mask_email_fails_when_secret_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("BLINDLOG_SECRET", raising=False)
    clear_logger_cache()
    with pytest.raises(MaskingError, match="BLINDLOG_SECRET"):
...
def test_debug_mode_is_refused(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_mask_email_and_phone_change_values() -> None:
...
def test_mask_payload_never_returns_original_pii() -> None:
...
def test_mask_payload_rejects_non_dict() -> None:
    with pytest.raises(MaskingError, match="dict"):
        mask_payload("not-a-dict")  # type: ignore[arg-type]


def test_empty_email_and_phone_are_rejected() -> None:
...
def test_spoofed_masked_email_is_still_hashed() -> None:
...
def test_spoofed_blind_prefix_phone_is_still_hashed() -> None:
    spoofed = "blind:+919876543210"
    masked = mask_phone(spoofed)
    assert masked != spoofed


def test_logger_cache_is_keyed_by_secret() -> None:
