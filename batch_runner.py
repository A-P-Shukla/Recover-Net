...
Usage:
    ...
    uv run python batch_runner.py --url http://localhost:8000 --concurrency 20
"""
...
class BatchReport:
    ...
    results: List[TxResult]           = field(default_factory=list)
...
def _expected_ok(label: str, escalated: int, total: int) -> bool:
    if total == 0:
        return True
    if label in ("fraud", "high_risk"):
...
def main() -> None:
    ...
    try:
        ...
        print("Run  `uv run python generate_batch.py`  first.")
        sys.exit(1)
