...
from blindlog import BlindLogConfig, BlindLogger  # pyright: ignore[reportMissingTypeStubs]
from blindlog.rules import MASKED_PATTERN  # pyright: ignore[reportMissingTypeStubs]
...
@contextmanager
def using_secret(secret_key: str) -> Generator[None, None, None]:
...
def mask_email(email: Optional[str], secret_key: Optional[str] = None) -> Optional[str]:
    ...
    original = str(email)
    logger: Any = get_blind_logger(secret_key)
    ...
    masked_email = cast(Dict[str, Any], result)["user_email"]
    return _masked_or_raise(original, str(masked_email), "user_email")
...
def mask_phone(phone: Optional[str], secret_key: Optional[str] = None) -> Optional[str]:
    ...
    masked_phone = cast(Dict[str, Any], result)["phone"]
    return _masked_or_raise(original, str(masked_phone), "phone")


def mask_payload(payload: Any, secret_key: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    masked = logger.mask(payload_dict)
    if not isinstance(masked, dict):
        raise MaskingError(
            "BlindLog did not return a dict; refusing to store unmasked payload"
        )

    result: Dict[str, Any] = cast(Dict[str, Any], masked).copy()
