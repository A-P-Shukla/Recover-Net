...
Usage:
    from workflow import run_recovery_pipeline, RecoveryResult
...
from classifier import RecoveryDecision, RecoveryIntent, classify_payment_failure
from guardrail import GuardrailResult, evaluate_action
from models import AuditLog, Transaction

logger = logging.getLogger("recover_net.workflow")

# Maps RecoveryIntent values to the final_status labels stored in audit_logs
_INTENT_TO_STATUS: Dict[str, str] = {
    "retry_now": "RETRIED",
...
def run_recovery_pipeline(
    ...
    Steps:
        ...
        6. Commit the transaction + audit log atomically.
    ...
    # ------------------------------------------------------------------
    # Pass original raw numeric fields (amount, past_success_rate,
    # error_code) — these are not PII and are needed for rule evaluation.
    guardrail_input = {
    ...
    guardrail_result: GuardrailResult = evaluate_action(
        guardrail_input, groq_decision.intent
    )
    ...
    # ------------------------------------------------------------------
    final_status = _INTENT_TO_STATUS.get(
