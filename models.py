...
from security import MaskingError, mask_email, mask_payload, mask_phone, using_secret
...
class Transaction(Base):
    ...
    def _from_webhook(
        ...
        tx = cls(
            ...
            past_success_rate=(
                float(raw_data.get("past_success_rate"))
                if raw_data.get("past_success_rate") is not None
                else None
            ),
        ...
        if tx.transaction_id is not None and source_id and str(tx.transaction_id) == source_id:
...
class AuditLog(Base):
    ...
    final_status: Mapped[str] = mapped_column(
        ...
        nullable=False,
        index=True,
        doc="Final status of the decision workflow (e.g. APPROVED, REJECTED, RETRIED, ESCALATED)",
    )
